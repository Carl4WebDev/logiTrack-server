...
const insertSampleShipment = async () => {
  const pool = new Pool({
    user: "postgres",
    password: "root",
    host: "localhost",
  ...
  try {
    ...
    await client.query("TRUNCATE TABLE shipments RESTART IDENTITY");
    ...
    const sampleShipment = {
      ...
      updatedBy: "Admin",
      status: "completed",
      file_name: "shipments-sample-1.xlsx",
    ...
    const queryValues = [
      sampleShipment.name,
      sampleShipment.description,
      sampleShipment.createdAt,
      sampleShipment.createdBy,
      sampleShipment.updatedAt,
      sampleShipment.updatedBy,
      sampleShipment.status,
      sampleShipment.file_name,
      sampleShipment.file_data,
    ];
